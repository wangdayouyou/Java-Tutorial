...
![Database - Instance](../../resource/img/Database%20-%20Instance.jpg)
...
![Logical-View-of-MySQL-Architecture](../../resource/img/Logical-View-of-MySQL-Architecture.jpg)
...
![Tablespace-segment-extent-page-row](../../resource/img/Tablespace-segment-extent-page-row.jpg)

同一个数据库实例的所有表空间都有相同的页大小；默认情况下，表空间中的页大小都为 16KB，当然也可以通过改变 `innodb_page_size` 选项对默认大小进行修改，需要注意的是不同的页大小最终也会导致区大小的不同：

![Relation Between Page Size - Extent Size](../../resource/img/Relation%20Between%20Page%20Size%20-%20Extent%20Size.png)
...
![frm-and-ibd-file](../../resource/img/frm-and-ibd-file.jpg)
...
![frm-file-hex](../../resource/img/frm-file-hex.png)
...
![Antelope-Barracuda-Row-Format](../../resource/img/Antelope-Barracuda-Row-Format.jpg)

> Antelope 是 InnoDB 最开始支持的文件格式，它包含两种行格式 Compact 和 Redundant，它最开始并没有名字；Antelope 的名字是在新的文件格式 Barracuda 出现后才起的，Barracuda 的出现引入了两种新的行格式 Compressed 和 Dynamic；InnoDB 对于文件格式都会向前兼容，而官方文档中也对之后会出现的新文件格式预先定义好了名字：Cheetah、Dragon、Elk 等等。

两种行记录格式 Compact 和 Redundant 在磁盘上按照以下方式存储：

![COMPACT-And-REDUNDANT-Row-Format](../../resource/img/COMPACT-And-REDUNDANT-Row-Format.jpg)
...
![Row-Overflo](../../resource/img/mysql/Row-Overflow.jpg)

但是当我们使用新的行记录格式 Compressed 或者 Dynamic 时都只会在行记录中保存 20 个字节的指针，实际的数据都会存放在溢出页面中。

![Row-Overflow-in-Barracuda](../../resource/img/Row-Overflow-in-Barracuda.jpg)
...
![InnoDB-B-Tree-Node](../../resource/img/InnoDB-B-Tree-Node.jpg)
...
![Infimum-Rows-Supremum](../../resource/img/Infimum-Rows-Supremum.jpg)
...
![B+Tree](../../resource/img/B+Tree.jpg)
...
![Clustered-Index](../../resource/img/Clustered-Index.jpg)
...
![Secondary-Index](../../resource/img/Secondary-Index.jpg)

如果在表 `users` 中存在一个辅助索引 `(first_name, age)`，那么它构成的 B+ 树大致就是上图这样，按照 `(first_name, age)` 的字母顺序对表中的数据进行排序，当查找到主键时，再通过聚集索引获取到整条行记录。

![Clustered-Secondary-Index](../../resource/img/Clustered-Secondary-Index.jpg)
...
![Optimistic-Pessimistic-Locks](../../resource/img/Optimistic-Pessimistic-Locks.jpg)
...
![Shared-Exclusive-Lock](../../resource/img/Shared-Exclusive-Lock.jpg)
...
![Lock-Type-Compatibility-Matrix](../../resource/img/Lock-Type-Compatibility-Matrix.jpg)
...
![Deadlocks](../../resource/img/Deadlocks.jpg)
...
![Transaction-Isolation-Matrix](../../resource/img/Transaction-Isolation-Matrix.jpg)
...
![Read-Uncommited-Dirty-Read](../../resource/img/Read-Uncommited-Dirty-Read.jpg)
...
![Read-Commited-Non-Repeatable-Read](../../resource/img/Read-Commited-Non-Repeatable-Read.jpg)
...
![Repeatable-Read-Phantom-Read](../../resource/img/Repeatable-Read-Phantom-Read.jpg)

这种现象在数据库中就被称作幻读，虽然我们使用查询语句得到了一个空的集合，但是插入数据时却得到了错误，好像之前的查询是幻觉一样。

在标准的事务隔离级别中，幻读是由更高的隔离级别 `SERIALIZABLE` 解决的，但是它也可以通过 MySQL 提供的 Next-Key 锁解决：

![Repeatable-with-Next-Key-Lock](../../resource/img/Repeatable-with-Next-Key-Lock.jpg)
